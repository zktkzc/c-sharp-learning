...
namespace 字符串加解密
{
    public partial class Form1 : Form
    {
        ...
        {
            string strKey = texEncKey.Text.Trim();
            if (strKey.Length != 4)
            {
                MessageBox.Show("密钥必须是4位！！！");
                return;
            }
            texEnc.Text = ToEncrypt(strKey, texOrigin.Text.Trim());
        }
        ...
        {
            string strKey = texDecKey.Text.Trim();
            if (strKey.Length != 4)
            {
                MessageBox.Show("解密密钥必须是4位！！！");
                return;
            }
            textDec.Text = ToDecrypt(strKey, texEnc2.Text.Trim());
        }
    }
}
